...
import 'item_property_list_item_card.dart';

class ItemPropertyListSectionHeader extends StatelessWidget {
  const ItemPropertyListSectionHeader({required this.text, super.key});
...
class ItemPropertyRecordListView extends StatelessWidget {
  const ItemPropertyRecordListView({
  ...
  final void Function(ItemPropertyCatalogEntry entry) onItemPrimaryTap;
  final void Function(ItemPropertyCatalogEntry entry)? onItemLongPress;
  ...
  String _emptyMessage() {
    if (totalItems == 0) {
      return 'No item properties yet.';
    }
    if (hasActiveSearch) {
      return 'No item properties match your search.';
    }
    return 'No item properties match these filters.';
  }
      ...
      builder: (context, constraints) {
        ...
        final desiredColumns =
            ((availableWidth + itemSpacing) / (minItemWidth + itemSpacing))
                .floor()
                .clamp(1, 3);
        final rowEntries = buildItemPropertyRowEntries(entries, desiredColumns);

        Widget rowAt(int index) {
          final rowEntry = rowEntries[index];
          if (rowEntry.header != null) {
            return ItemPropertyListSectionHeader(text: rowEntry.header!);
          }
          return Row(
            ...
            children: [
              for (var i = 0; i < rowEntry.entries.length; i++) ...[
                ...
                Expanded(
                  child: ItemPropertyListItemCard(
                    property: rowEntry.entries[i].entry,
