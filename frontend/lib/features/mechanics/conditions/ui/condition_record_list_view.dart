...
class ConditionListSectionHeader extends StatelessWidget {
  const ConditionListSectionHeader({required this.text, super.key});
...
class ConditionRecordListView extends StatelessWidget {
  const ConditionRecordListView({
  ...
  final void Function(ConditionCatalogEntry entry) onItemPrimaryTap;
  final void Function(ConditionCatalogEntry entry)? onItemLongPress;
  ...
  String _emptyMessage() {
    ...
    if (hasActiveSearch) {
      return 'No conditions match your search.';
    }
    return 'No conditions match these filters.';
  }
      ...
      builder: (context, constraints) {
        ...
        final desiredColumns =
            ((availableWidth + itemSpacing) / (minItemWidth + itemSpacing))
                .floor()
                .clamp(1, 3);
        final rowEntries = buildConditionRowEntries(entries, desiredColumns);

        Widget rowAt(int index) {
          final rowEntry = rowEntries[index];
          if (rowEntry.header != null) {
            return ConditionListSectionHeader(text: rowEntry.header!);
          }
          return Row(
              ...
              for (var i = 0; i < rowEntry.entries.length; i++) ...[
                ...
                Expanded(
                  child: ConditionListItemCard(
                    record: rowEntry.entries[i].entry,
