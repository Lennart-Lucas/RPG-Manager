...
class TransformationsBody extends StatefulWidget {
  const TransformationsBody({super.key, required this.auth});

  final AuthController auth;

  @override
  State<TransformationsBody> createState() => _TransformationsBodyState();
}

class _TransformationsBodyState extends State<TransformationsBody> {
  ...
  Future<void> _reload() async {
    ...
    try {
      ...
      final items = await _api.list(token, CatalogKind.transformations);
      if (!mounted) return;
      setState(() {
        _items = items;
        _loading = false;
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Could not load transformations';
        _loading = false;
      });
    }
  }
  ...
  Future<void> _create() async {
    try {
      ...
      final record = await showTransformationFormSheet(
        context,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not create transformation')),
      );
    }
  }
          ...
          RefreshIndicator(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final record = TransformationRecord.fromCatalogPayload(
                  name: item.name,
