...
class RacesBody extends StatefulWidget {
  const RacesBody({super.key, required this.auth});
...
class _RacesBodyState extends State<RacesBody> {
  ...
  Future<void> _reload() async {
    ...
    try {
      ...
      final items = await _api.list(token, CatalogKind.races);
      if (!mounted) return;
      setState(() {
        _items = items;
        _loading = false;
  ...
  Future<void> _create() async {
    try {
      ...
      final race = await showRaceFormSheet(
        context,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not create race')),
      );
    }
  }
  ...
  Future<void> _open(CatalogItem item) async {
    final deleted = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (context) => RaceDetailPage(auth: widget.auth, item: item),
      ),
    );
        ...
        Positioned.fill(
          child: IgnorePointer(
            child: Center(
              child: Opacity(
                opacity: 0.08,
                child: Icon(racesPageIcon, size: 440, color: scheme.onSurface),
              ),
            ),
          ),
        ),
          ...
          RefreshIndicator(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final race = RaceRecord.fromCatalogPayload(
                  name: item.name,
