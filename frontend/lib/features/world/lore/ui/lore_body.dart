...
class LoreBody extends StatefulWidget {
  const LoreBody({super.key, required this.auth});
...
class _LoreBodyState extends State<LoreBody> {
  ...
  Future<void> _reload() async {
    ...
    try {
      ...
      final items = await _api.list(token, CatalogKind.lore);
      if (!mounted) return;
      setState(() {
        _items = items;
        _loading = false;
  ...
  Future<void> _create() async {
    try {
      ...
      final record = await showLoreFormSheet(
        context,
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not create lore')),
      );
    }
  }
        ...
        Positioned.fill(
          child: IgnorePointer(
            child: Center(
              child: Opacity(
                opacity: 0.08,
                child: Icon(lorePageIcon, size: 440, color: scheme.onSurface),
              ),
            ),
          ),
        ),
                          ...
                          children: [
                            ...
                            Text(
                              widget.auth.canMutateCatalog
                                  ? 'Tap + to add your first lore entry.'
                                  : 'No lore yet.',
                              textAlign: TextAlign.center,
          ...
          RefreshIndicator(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final record = LoreRecord.fromCatalogPayload(
                  name: item.name,
